...
class BackgroundUpload: RCTEventEmitter, URLSessionDelegate, URLSessionTaskDelegate, URLSessionDataDelegate {
  ...
  @objc func uploadFile(
    ...
    guard let url = URL(string: urlString) else {
      reject("INVALID_URL", "Invalid upload URL", nil); return
    }
    guard let fileURL = URL(string: fileUri) else {
      reject("INVALID_FILE", "Invalid file URI", nil); return
    }
    ...
    let tempFile = FileManager.default.temporaryDirectory.appendingPathComponent("\(uploadId).tmp")
    ...
    do {
      ...
      reject("FILE_ERROR", "Could not read file: \(error.localizedDescription)", error); return
    }
  ...
  @objc func checkPendingUploads(
    ...
    for (key, value) in all {
      if key.hasPrefix("upload_"), let dict = value as? [String: String] {
        var entry = dict
        entry["uploadId"] = String(key.dropFirst(7))
  ...
  func urlSession(_ session: URLSession, dataTask: URLSessionDataTask, didReceive data: Data) {
    ...
    if responseData[taskId] == nil { responseData[taskId] = Data() }
